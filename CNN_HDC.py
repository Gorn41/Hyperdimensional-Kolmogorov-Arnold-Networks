...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(500, 1000)  # Adjusted to match output size
        self.relu = nn.ReLU()
...
def test(model, testloader, device):
    ...
    plt.ylabel('Actual')
    plt.title('HDC CNN Confusion Matrix (No Noise)')
    plt.savefig("./cnnhdc_fashionmnist_confusion_matrix_no_noise.png")
    plt.show()
...
def test_with_noise(model, testloader, device, noise_std=0.1):
    ...
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.title(f'HDC-CNN Confusion Matrix (Noise Std = {noise_std})')
    plt.savefig(f"./hdc_cnn_fashionmnist_confusion_matrix_noise_{noise_std}.png")
    plt.show()

    # Classification Report
    print("Classification Report:")
    print(classification_report(all_labels, all_preds))
    with open(f"./hdc_cnn_baseline_classification_report_noise_{noise_std}.txt", 'a', newline='') as file:
...
def main(trainingmode=True):
    if trainingmode:
        ...
        torch.save(best_model.state_dict(), "models/cnn_hdc_fashion_MNIST.pth")
