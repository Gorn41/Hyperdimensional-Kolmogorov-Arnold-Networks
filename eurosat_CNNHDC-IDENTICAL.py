...
def main():
    ...
    model.feature_network.load_state_dict(torch.load("./Eurosat_results/baselineCNN_IDENT_best.pth", map_location=device))

    model.train_lehdc(train_loader, valloader)

    model.eval()

    test("HDCCNN-IDENT", "Eurosat_results", model, test_loader, device)
    test_with_noise("HDCCNN-IDENT", "Eurosat_results", model, test_loader, device, noise_std=0.1)
    test_with_noise("HDCCNN-IDENT", "Eurosat_results", model, test_loader, device, noise_std=0.4)
    test_with_noise("HDCCNN-IDENT", "Eurosat_results", model, test_loader, device, noise_std=0.7)
    test_with_noise("HDCCNN-IDENT", "Eurosat_results", model, test_loader, device, noise_std=1.0)
