...
class KANCLeHDCModel(nn.Module):
    def __init__(self, n_dimensions=10000, n_classes=10, n_levels=200, grid_size=5):
...
def train(model, data, learning_rate, epochs, device, val_loader):
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        # plt.figure()
        # plt.plot(np.arange(1, epoch + 1), np.array(accs))
        # plt.xlabel("Epoch")
        # plt.ylabel("Validation Accuracy")
        # plt.title("CNN Baseline Validation Accuracy over Epochs")
        # plt.savefig("./cnn_baseline_fashionmnist_val_acc.png")
        # plt.figure()
        # plt.plot(np.arange(1, epochs + 1), np.array(losses), label='Training Loss')
        # plt.plot(np.arange(1, epochs + 1), np.array(val_losses), label='Validation Loss')
        # plt.xlabel("Epoch")
        # plt.ylabel("Loss")
