...
def train(model, data, learning_rate, epochs, device, val_loader):
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        plt.title("CNN Global Pooling baseline Validatation Accuracy over Epochs")
        plt.savefig("./cnn_globalpooling_fashionmnist_val_acc.png")
        plt.figure()
        plt.plot(np.arange(1, epoch + 2), losses)
        plt.xlabel("Epoch")
        plt.ylabel("Training Loss")
        plt.title("CNN Global Pooling baseline Training Loss over Epochs")
        plt.savefig("./cnn_globalpooling_fashionmnist_training_loss.png")
        plt.figure()
        plt.plot(np.arange(1, epoch + 2), val_losses)
        plt.xlabel("Epoch")
        plt.title("CNN Global Pooling baseline Validatation Loss over Epochs")
        plt.ylabel("Validation Loss")
        plt.savefig("./cnn_globalpooling_fashionmnist_val_loss.png")
        plt.close('all')
...
def test(model, testloader, device):
    ...
    plt.ylabel('Actual')
    plt.title('CNN Global Pooling Confusion Matrix (No Noise)')
    plt.savefig("./cnn_globalpooling_fashionmnist_confusion_matrix_no_noise.png")
    plt.show()

    print("Classification Report:")
    print(classification_report(all_labels, all_preds))
    with open("./cnn_globalpooling_classification_report_no_noise.txt", 'a', newline='') as file:
...
def test_with_noise(model, testloader, device, noise_std=0.1):
    ...
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.title(f'CNN Global Pooling Confusion Matrix (Noise Std = {noise_std})')
